...
void publishIMUData() {
  ...
  if (success) {
    ...
    // scale_start = micros();
    imu_msg.angular_velocity.x = imu_data[1];
    imu_msg.angular_velocity.y = imu_data[2];
    imu_msg.angular_velocity.z = imu_data[3];

    imu_msg.linear_acceleration.x = imu_data[4];
    imu_msg.linear_acceleration.y = imu_data[5];
    imu_msg.linear_acceleration.z = imu_data[6];
